...
public class EnemyBlack1Infinite : MonoBehaviour
{
    [SerializeField] List<Sprite> sprites;
    [SerializeField] List<Transform> pathOneWaypoints;
    ...
    //use a 'mini explosion' to indicate damage to enemy vs the red animation
    public int waypointIndex = 0, hitsAllowed = 3, hits = 0;
    public bool hitNow, allowHit,start,pathOne1,pathOne2,pathOne3,instantiated;
    ...
    void Update()
    {
        ...
        if(start) {
            ...
            if(pathOne1) {
            var targetPosition = pathOneWaypoints[waypointIndex].transform.position;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if(other.gameObject.tag == "PlayerLaser" && !dead) {
            ...
            if(hits >= hitsAllowed && !dead) {
                ...
                InfiniteInstantiatingManager.pathOneEnemiesDestroyed++;
