...
public class EnemyBlack2Infinite : MonoBehaviour
{
    [SerializeField] List<Sprite> sprites;
    [SerializeField] List<Transform> pathTwoWaypoints;
    ...
    public bool allowHit,start,pathTwo1,pathTwo2,pathTwo3,instantiated;
    bool dead;
    void Awake()
    {
        allowHit = true;
    ...
    void Update()
    {
        ...
        if(start) {
            ...
            if(pathTwo1) {
            var targetPosition = pathTwoWaypoints[waypointIndex].transform.position;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if(other.gameObject.tag == "PlayerLaser" && !dead) {
            ...
            if(hits >= hitsAllowed && !dead) {
                ...
                myAudioSource.PlayOneShot(clip);
                InfiniteInstantiatingManager.pathTwoEnemiesDestroyed++;
